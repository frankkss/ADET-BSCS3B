...
def dash():
    ...
    try:
        ...
        encryptedPW = hashlib.sha256(info['Password'].encode()).hexdigest()  #add encrption here
        sequel_query = f'SELECT Username, f_name, m_name, l_name, c_number, email, address FROM USER");'
        # return sequel_query
        cur.execute(sequel_query)
        dbData = cur.fetchall()
